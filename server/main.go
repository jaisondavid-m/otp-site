...
func main() {
	...
	api.Use(middleware.RequireSession(db))
	{
		...
		api.GET("/pending-action/v2", h.ProxyPendingActionV2)
		api.POST("/pending-action", h.ProxySubmitPendingAction)
		api.GET("/activity", h.ProxyActivity)
		api.POST("/activity", h.ProxyCreateActivity)
		api.GET("/activity/details", h.ProxyActivityDetails)
		api.POST("/activity/start-activity", h.ProxyStartActivity)
		api.POST("/activity/add-participants", h.ProxyAddParticipants)
		api.POST("/activity/end-activity", h.ProxyEndActivity)
		api.POST("/activity/transfer", h.ProxyTransferActivity)
	...
	admin.Use(middleware.RequireAdmin())
	{
		...
		admin.POST("/users/:device_id/quick-login", h.UpdateUserQuickLogin)
		admin.PATCH("/users/:device_id/quick-login", h.UpdateUserQuickLogin)
